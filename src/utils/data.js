import AMBULAN from "../img/semprot.jpg";
import SERTI1 from "../img/serti1.jpg";
import SERTI2 from "../img/serti2.jpg";
import SERTI3 from "../img/serti3.jpg";
import SERTI4 from "../img/serti4.jpg";
import SERTI5 from "../img/serti5.jpg";
import SERTI6 from "../img/serti6.jpg";
import SERTI7 from "../img/serti7.jpg";
import SERTI8 from "../img/serti8.jpg";
import SERTI9 from "../img/serti9.jpg";
import SERTI10 from "../img/serti10.jpg";
import SERTI11 from "../img/serti11.jpg";
import SERTI12 from "../img/serti12.jpg";
import SERTI13 from "../img/serti13.jpg";
import DELIMA from "../img/delima.png";
import BAHTERA from "../img/bahtera.png";
import AUTO from "../img/autoassembler.png";
import l1 from "../img/1.jpg";
import l2 from "../img/2.jpg";
import l3 from "../img/3.jpg";
import l4 from "../img/4.jpg";
import l5 from "../img/5.jpg";
import l6 from "../img/6.jpg";
import l7 from "../img/7.jpg";
import l8 from "../img/8.jpg";
import l9 from "../img/9.jpg";
import l10 from "../img/10.jpg";
import l11 from "../img/11.jpg";
import l12 from "../img/12.jpg";
import l13 from "../img/13.jpg";
import l14 from "../img/14.jpg";
import l15 from "../img/15.jpg";
import l16 from "../img/16.jpg";
import l17 from "../img/17.jpg";
import l18 from "../img/18.jpg";
import l19 from "../img/19.jpg";
import l20 from "../img/10.jpg";
import l21 from "../img/21.jpg";
import l22 from "../img/22.jpg";
import l23 from "../img/23.jpg";
import l24 from "../img/24.jpg";
import l25 from "../img/25.jpg";
import l26 from "../img/26.jpg";
import l27 from "../img/27.jpg";
import l28 from "../img/28.jpg";
import l29 from "../img/29.jpg";
import l30 from "../img/10.jpg";
import l31 from "../img/31.jpg";
import l32 from "../img/32.jpg";
import l33 from "../img/33.jpg";
import l34 from "../img/34.jpg";
import l35 from "../img/35.jpg";
import l36 from "../img/36.jpg";
import l37 from "../img/37.jpg";
import l38 from "../img/38.jpg";
import l39 from "../img/39.jpg";
import l40 from "../img/10.jpg";
import l41 from "../img/41.jpg";
import l42 from "../img/42.jpg";
import l43 from "../img/43.jpg";
import l44 from "../img/44.jpg";
import l45 from "../img/45.jpg";
import l46 from "../img/46.jpg";
import l47 from "../img/47.jpg";
import l48 from "../img/48.jpg";
import l49 from "../img/49.jpg";
import l50 from "../img/50.jpg";
import l51 from "../img/51.jpg";
import l52 from "../img/52.jpg";
import l53 from "../img/53.jpg";
import l54 from "../img/54.jpg";
import l55 from "../img/55.jpg";
import l56 from "../img/56.jpg";
import l57 from "../img/57.jpg";
import l58 from "../img/58.jpg";
import l59 from "../img/59.jpg";
import l60 from "../img/60.jpg";
import l61 from "../img/61.jpg";
import l62 from "../img/62.jpg";
import l63 from "../img/63.jpg";
import l64 from "../img/64.jpg";
import l65 from "../img/65.jpg";
import l66 from "../img/66.jpg";
import l67 from "../img/67.jpg";
import l68 from "../img/68.jpg";
import LogoDelima from "../img/logo delima.png";
import Blibli from "../img/blibli.png";
import Blibli1 from "../img/blibli1.png";
import Blibli2 from "../img/Impact.png";
import Manhauler from "../img/manhauler.png";

export const heroData = [
  {
    id: 1,
    name: "Ambulan",
    imageSrc: AMBULAN,
  },
  {
    id: 2,
    name: "Serti1",
    imageSrc: SERTI1,
  },
  {
    id: 3,
    name: "Serti2",
    imageSrc: SERTI2,
  },
  {
    id: 4,
    name: "Serti3",
    imageSrc: SERTI3,
  },
  {
    id: 5,
    name: "Serti4",
    imageSrc: SERTI4,
  },
  {
    id: 6,
    name: "Serti5",
    imageSrc: SERTI5,
  },
  {
    id: 6,
    name: "Serti6",
    imageSrc: SERTI6,
  },
  {
    id: 7,
    name: "serti7",
    imageSrc: SERTI7,
  },
  {
    id: 8,
    name: "serti8",
    imageSrc: SERTI8,
  },
  {
    id: 9,
    name: "serti9",
    imageSrc: SERTI9,
  },
  {
    id: 10,
    name: "serti10",
    imageSrc: SERTI10,
  },
  {
    id: 11,
    name: "serti11",
    imageSrc: SERTI11,
  },
  {
    id: 12,
    name: "serti12",
    imageSrc: SERTI12,
  },
  {
    id: 13,
    name: "serti13",
    imageSrc: SERTI13,
  },
  {
    id: 14,
    name: "delima",
    imageSrc: DELIMA,
  },
  {
    id: 15,
    name: "bahtera",
    imageSrc: BAHTERA,
  },
  {
    id: 16,
    name: "auto",
    imageSrc: AUTO,
  },
  {
    id: 17,
    name: "l1",
    imageSrc: l1,
  },
  {
    id: 18,
    name: "l2",
    imageSrc: l2,
  },
  {
    id: 19,
    name: "l3",
    imageSrc: l3,
  },
  {
    id: 20,
    name: "l4",
    imageSrc: l4,
  },
  {
    id: 21,
    name: "l5",
    imageSrc: l5,
  },
  {
    id: 22,
    name: "l6",
    imageSrc: l6,
  },
  {
    id: 23,
    name: "l7",
    imageSrc: l7,
  },
  {
    id: 24,
    name: "l8",
    imageSrc: l8,
  },
  {
    id: 25,
    name: "l9",
    imageSrc: l9,
  },
  {
    id: 26,
    name: "l10",
    imageSrc: l10,
  },
  {
    id: 27,
    name: "l11",
    imageSrc: l11,
  },
  {
    id: 28,
    name: "l12",
    imageSrc: l12,
  },
  {
    id: 29,
    name: "l13",
    imageSrc: l13,
  },
  {
    id: 30,
    name: "l14",
    imageSrc: l14,
  },
  {
    id: 31,
    name: "l15",
    imageSrc: l15,
  },
  {
    id: 32,
    name: "l16",
    imageSrc: l16,
  },
  {
    id: 33,
    name: "l17",
    imageSrc: l17,
  },
  {
    id: 34,
    name: "l18",
    imageSrc: l18,
  },
  {
    id: 35,
    name: "l19",
    imageSrc: l19,
  },
  {
    id: 36,
    name: "l20",
    imageSrc: l20,
  },
  {
    id: 37,
    name: "l21",
    imageSrc: l21,
  },
  {
    id: 38,
    name: "l22",
    imageSrc: l22,
  },
  {
    id: 39,
    name: "l23",
    imageSrc: l23,
  },
  {
    id: 40,
    name: "l24",
    imageSrc: l24,
  },
  {
    id: 41,
    name: "l25",
    imageSrc: l25,
  },
  {
    id: 42,
    name: "l26",
    imageSrc: l26,
  },
  {
    id: 43,
    name: "l27",
    imageSrc: l27,
  },
  {
    id: 44,
    name: "l28",
    imageSrc: l28,
  },
  {
    id: 45,
    name: "l29",
    imageSrc: l29,
  },
  {
    id: 46,
    name: "l30",
    imageSrc: l30,
  },
  {
    id: 47,
    name: "l31",
    imageSrc: l31,
  },
  {
    id: 48,
    name: "l32",
    imageSrc: l32,
  },
  {
    id: 49,
    name: "l33",
    imageSrc: l33,
  },
  {
    id: 50,
    name: "l34",
    imageSrc: l34,
  },
  {
    id: 51,
    name: "l35",
    imageSrc: l35,
  },
  {
    id: 52,
    name: "l36",
    imageSrc: l36,
  },
  {
    id: 53,
    name: "l37",
    imageSrc: l37,
  },
  {
    id: 54,
    name: "l38",
    imageSrc: l38,
  },
  {
    id: 55,
    name: "l39",
    imageSrc: l39,
  },
  {
    id: 56,
    name: "l40",
    imageSrc: l40,
  },
  {
    id: 57,
    name: "l51",
    imageSrc: l51,
  },
  {
    id: 58,
    name: "l52",
    imageSrc: l52,
  },
  {
    id: 59,
    name: "l53",
    imageSrc: l53,
  },
  {
    id: 60,
    name: "l54",
    imageSrc: l54,
  },
  {
    id: 71,
    name: "l55",
    imageSrc: l55,
  },
  {
    id: 72,
    name: "l56",
    imageSrc: l56,
  },
  {
    id: 73,
    name: "l57",
    imageSrc: l57,
  },
  {
    id: 74,
    name: "l58",
    imageSrc: l58,
  },
  {
    id: 75,
    name: "l59",
    imageSrc: l59,
  },
  {
    id: 76,
    name: "l60",
    imageSrc: l60,
  },
  {
    id: 77,
    name: "l61",
    imageSrc: l61,
  },
  {
    id: 78,
    name: "l62",
    imageSrc: l62,
  },
  {
    id: 79,
    name: "l63",
    imageSrc: l63,
  },
  {
    id: 80,
    name: "l64",
    imageSrc: l64,
  },
  {
    id: 81,
    name: "l65",
    imageSrc: l65,
  },
  {
    id: 82,
    name: "l66",
    imageSrc: l66,
  },
  {
    id: 83,
    name: "l67",
    imageSrc: l67,
  },
  {
    id: 84,
    name: "l68",
    imageSrc: l68,
  },
  {
    id: 85,
    name: "l41",
    imageSrc: l41,
  },
  {
    id: 86,
    name: "l42",
    imageSrc: l42,
  },
  {
    id: 87,
    name: "l43",
    imageSrc: l43,
  },
  {
    id: 88,
    name: "l44",
    imageSrc: l44,
  },
  {
    id: 89,
    name: "l45",
    imageSrc: l45,
  },
  {
    id: 90,
    name: "l46",
    imageSrc: l46,
  },
  {
    id: 91,
    name: "l47",
    imageSrc: l47,
  },
  {
    id: 92,
    name: "l48",
    imageSrc: l48,
  },
  {
    id: 93,
    name: "l49",
    imageSrc: l49,
  },
  {
    id: 94,
    name: "l50",
    imageSrc: l50,
  },
  {
    id: 95,
    name: "LogoDelima",
    imageSrc: LogoDelima,
  },
  {
    id: 96,
    name: "blibli",
    imageSrc: Blibli,
  },
  {
    id: 97,
    name: "blibli1",
    imageSrc: Blibli1,
  },
  {
    id: 98,
    name: "blibli2",
    imageSrc: Blibli2,
  },
  {
    id: 99,
    name: "Manhauler",
    imageSrc: Manhauler,
  },
];

export const categories = [
  {
    id: 1,
    name: "Pelayanan Kas",
    urlParamName: "pelayanan_kas",
    subcategories: [
      { id: "1-1", name: "Merci" },
      { id: "1-2", name: "App" },
      { id: "1-3", name: "Truk" },
      { id: "1-4", name: "Bis" },
      { id: "1-5", name: "Bus" },
      { id: "1-6", name: "Motor" },
    ],
  },
  {
    id: 2,
    name: "Cabin Operator",
    urlParamName: "cabin_operator",
    subcategories: [{ id: "2-1", name: "Truk" }],
  },
  {
    id: 3,
    name: "Mobil Khusus",
    urlParamName: "mobil_khusus",
    subcategories: [],
  },
  {
    id: 4,
    name: "Ambulance",
    urlParamName: "ambulance",
    subcategories: [],
  },
  {
    id: 5,
    name: "VIP Partition",
    urlParamName: "vip_partition",
    subcategories: [],
  },
  {
    id: 6,
    name: "Luxury Interior",
    urlParamName: "luxury_interior",
    subcategories: [],
  },
  {
    id: 7,
    name: "Buggy Wisata",
    urlParamName: "buggy_wisata",
    subcategories: [],
  },
  {
    id: 8,
    name: "Box Bak Kargo",
    urlParamName: "box_bak_kargo",
    subcategories: [],
  },
  {
    id: 9,
    name: "Skylift Crane",
    urlParamName: "skylift_crane",
    subcategories: [],
  },
  {
    id: 10,
    name: "Tanker Truck",
    urlParamName: "tanker_truck",
    subcategories: [],
  },
  {
    id: 11,
    name: "Taktis Defenser",
    urlParamName: "taktis_defense",
    subcategories: [],
  },
  {
    id: 12,
    name: "Service Rescue",
    urlParamName: "service_rescue",
    subcategories: [],
  },
  {
    id: 13,
    name: "Partition Wall",
    urlParamName: "partition_wall",
    subcategories: [],
  },
  {
    id: 14,
    name: "Mixer Concrete",
    urlParamName: "mixer_concrete",
    subcategories: [],
  },
  {
    id: 15,
    name: "Manhauler",
    urlParamName: "manhauler",
    subcategories: [],
  },
  {
    id: 16,
    name: "Medium Bus",
    urlParamName: "medium_bus",
    subcategories: [],
  },
  {
    id: 17,
    name: "Fabrikasi",
    urlParamName: "fabrikasi",
    subcategories: [],
  },
  {
    id: 18,
    name: "Fire truck",
    urlParamName: "fire_truck",
    subcategories: [],
  },
  {
    id: 19,
    name: "VIP Interior",
    urlParamName: "vip_nterior",
    subcategories: [],
  },
  {
    id: 20,
    name: "Microbus Mini",
    urlParamName: "microbus_mini",
    subcategories: [],
  },
  {
    id: 21,
    name: "Food Truck",
    urlParamName: "food_truck",
    subcategories: [],
  },
  {
    id: 22,
    name: "Mobil Derek Tow",
    urlParamName: "mobil_derek_tow",
    subcategories: [],
  },
  {
    id: 23,
    name: "Bull Bar Bumper",
    urlParamName: "bull_bar_bumper",
    subcategories: [],
  },
  {
    id: 24,
    name: "Dump Truck Tip",
    urlParamName: "dump_truck_tip",
    subcategories: [],
  },
  {
    id: 25,
    name: "Camver Van-RV",
    urlParamName: "camver_van_rv",
    subcategories: [],
  },
  {
    id: 26,
    name: "Container",
    urlParamName: "container",
    subcategories: [],
  },
  {
    id: 27,
    name: "Motor Roda 3r",
    urlParamName: "motor_roda_3",
    subcategories: [],
  },
  {
    id: 28,
    name: "Trailer-Semi",
    urlParamName: "trailer_semi",
    subcategories: [],
  },
  {
    id: 29,
    name: "Big Bus",
    urlParamName: "big_bus",
    subcategories: [],
  },
  {
    id: 30,
    name: "Freezer Box",
    urlParamName: "freezer_box",
    subcategories: [],
  },
  {
    id: 31,
    name: "Seat Product",
    urlParamName: "seat_product",
    subcategories: [],
  },
  {
    id: 32,
    name: "FAW Truck",
    urlParamName: "faw_truck",
    subcategories: [],
  },
  {
    id: 33,
    name: "FOR SALE",
    urlParamName: "for_sale",
    subcategories: [],
  },
];
